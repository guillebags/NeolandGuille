...
// -----------------> return explicito
const allCharacterMappedReturnExplicito = character.results.map(
  (personaje, index) => {
    return {
      index,
      name: personaje.name,
...
const filterData = character.results.filter(
  (element, index) => element.status === "Dead"
);
// console.log(filterData) // me saca todos los que estan con el estatus dead

const { results } = character;

const filterDataAliveComplex = results.filter(
  (element) =>
    element.status === "Dead" && element.name.toLowerCase().includes("a")
);
const filterMapped = filterDataAliveComplex.map((personaje) => ({
  name: personaje.name,
...
const singleCharacter = results.find((element, index) =>
  element.name.toLowerCase().includes("smith")
);
...
console.log(
  "🚀 ~ file: app.js:174 ~ totalPowerMutant ~ totalPowerMutant:",
  totalPowerMutant
);
